import Acer from "../../Assets/Images/product images/Home/computers/Acer Nitro.jpg";
import Lenovo from "../../Assets/Images/product images/Home/computers/Lenovo Ideapad.jpg";
import HP from "../../Assets/Images/product images/Home/computers/HP Laptop.jpg";
import Dell from "../../Assets/Images/product images/Home/computers/Dell.jpg";
import iMac from "../../Assets/Images/product images/Home/computers/Apple iMac.jpg";

const computersItems = [
  {
    id: 1,
    src: Acer,
    description:
      "Acer Nitro 5 AN515-55-53E5 Gaming Laptop | Intel Core i5-10300H",
    shelve: "computers",
    price: 70,
    currency: "SAR",
    period: "week",
  },

  {
    id: 2,
    src: Lenovo,
    description: "Lenovo Ideapad Laptop With FHD 11.6-Inch",
    shelve: "computers",
    price: 60,
    currency: "SAR",
    period: "week",
  },

  {
    id: 3,
    src: HP,
    description: 'HP Laptop 15-da2029nx 15.6" HD, Intel Core i3',
    shelve: "computers",
    price: 66,
    currency: "SAR",
    period: "week",
  },

  {
    id: 4,
    src: Dell,
    description:
      "Dell XPS 15 FHD+ Intel Core i7-11800H, GeForce RTX 3050Ti 4GB GDDR6",
    shelve: "computers",
    price: 89,
    currency: "SAR",
    period: "week",
  },

  {
    id: 5,
    src: iMac,
    description:
      "New Apple iMac Retina 5K Display (27-inch, 8GB RAM, 256GB SSD Storage)",
    shelve: "computers",
    price: 99,
    currency: "SAR",
    period: "week",
  },

  {
    id: 6,
    src: Acer,
    description:
      "Acer Nitro 5 AN515-55-53E5 Gaming Laptop | Intel Core i5-10300H",
    shelve: "computers",
    price: 70,
    currency: "SAR",
    period: "week",
  },

  {
    id: 7,
    src: Lenovo,
    description: "Lenovo Ideapad Laptop With FHD 11.6-Inch",
    shelve: "computers",
    price: 60,
    currency: "SAR",
    period: "week",
  },

  {
    id: 8,
    src: HP,
    description: 'HP Laptop 15-da2029nx 15.6" HD, Intel Core i3',
    shelve: "computers",
    price: 66,
    currency: "SAR",
    period: "week",
  },

  {
    id: 9,
    src: Dell,
    description:
      "Dell XPS 15 FHD+ Intel Core i7-11800H, GeForce RTX 3050Ti 4GB GDDR6",
    shelve: "computers",
    price: 89,
    currency: "SAR",
    period: "week",
  },

  {
    id: 10,
    src: iMac,
    description:
      "New Apple iMac Retina 5K Display (27-inch, 8GB RAM, 256GB SSD Storage)",
    shelve: "computers",
    price: 99,
    currency: "SAR",
    period: "week",
  },
];

export default computersItems;
